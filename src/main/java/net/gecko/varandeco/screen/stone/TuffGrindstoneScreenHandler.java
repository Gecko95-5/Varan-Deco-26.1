...
public class TuffGrindstoneScreenHandler extends ScreenHandler {
	...
	final Inventory input = new SimpleInventory(2) {
		...
		public void markDirty() {
			super.markDirty();
			TuffGrindstoneScreenHandler.this.onContentChanged(this);
		}
	};
	private final ScreenHandlerContext context;

	public TuffGrindstoneScreenHandler(int syncId, PlayerInventory playerInventory) {
		this(syncId, playerInventory, ScreenHandlerContext.EMPTY);
	}

	public TuffGrindstoneScreenHandler(int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
		...
		this.addSlot(new Slot(this.result, 2, 129, 34) {
			...
			public void onTakeItem(PlayerEntity player, ItemStack stack) {
				...
				TuffGrindstoneScreenHandler.this.input.setStack(0, ItemStack.EMPTY);
				TuffGrindstoneScreenHandler.this.input.setStack(1, ItemStack.EMPTY);
			}

			private int getExperience(World world) {
				int i = 0;
				i += this.getExperience(TuffGrindstoneScreenHandler.this.input.getStack(0));
				i += this.getExperience(TuffGrindstoneScreenHandler.this.input.getStack(1));
				if (i > 0) {
	...
	public boolean canUse(PlayerEntity player) {
		return canUse(this.context, player, DecoBlocks.TUFF_GRINDSTONE);
	}
