...
public class DeepslateGrindstoneScreenHandler extends AbstractContainerMenu {
	...
	final Container input = new SimpleContainer(2) {
		...
		public void setChanged() {
			super.setChanged();
			DeepslateGrindstoneScreenHandler.this.slotsChanged(this);
		}
	};
	private final ContainerLevelAccess context;

	public DeepslateGrindstoneScreenHandler(int syncId, Inventory playerInventory) {
		this(syncId, playerInventory, ContainerLevelAccess.NULL);
	}

	public DeepslateGrindstoneScreenHandler(int syncId, Inventory playerInventory, ContainerLevelAccess context) {
		...
		this.addSlot(new Slot(this.result, 2, 129, 34) {
			...
			public void onTake(Player player, ItemStack stack) {
				...
				DeepslateGrindstoneScreenHandler.this.input.setItem(0, ItemStack.EMPTY);
				DeepslateGrindstoneScreenHandler.this.input.setItem(1, ItemStack.EMPTY);
			}

			private int getExperience(Level world) {
				int i = 0;
				i += this.getExperience(DeepslateGrindstoneScreenHandler.this.input.getItem(0));
				i += this.getExperience(DeepslateGrindstoneScreenHandler.this.input.getItem(1));
				if (i > 0) {
	...
	public boolean stillValid(Player player) {
		return stillValid(this.context, player, DecoBlocks.DEEPSLATE_GRINDSTONE);
	}
