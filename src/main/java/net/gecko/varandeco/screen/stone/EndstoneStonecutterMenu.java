...
public class EndstoneStonecutterMenu extends AbstractContainerMenu {
    ...
    public final Container container = new SimpleContainer(1) {
        {
            Objects.requireNonNull(EndstoneStonecutterMenu.this);
        }

        @Override
        public void setChanged() {
            super.setChanged();
            EndstoneStonecutterMenu.this.slotsChanged(this);
            EndstoneStonecutterMenu.this.slotUpdateListener.run();
        }
    };
    private final ResultContainer resultContainer = new ResultContainer();

    public EndstoneStonecutterMenu(final int containerId, final Inventory inventory) {
        this(containerId, inventory, ContainerLevelAccess.NULL);
    }

    public EndstoneStonecutterMenu(final int containerId, final Inventory inventory, final ContainerLevelAccess access) {
        ...
        this.resultSlot = this.addSlot(new Slot(this.resultContainer, 1, 143, 33) {
            ...
            public void onTake(final Player player, final ItemStack carried) {
                ...
                EndstoneStonecutterMenu.this.resultContainer.awardUsedRecipes(player, this.getRelevantItems());
                ItemStack remaining = EndstoneStonecutterMenu.this.inputSlot.remove(1);
                if (!remaining.isEmpty()) {
                    EndstoneStonecutterMenu.this.setupResultSlot(EndstoneStonecutterMenu.this.selectedRecipeIndex.get());
                }

                access.execute((level, pos) -> {
                    long gameTime = level.getGameTime();
                    if (EndstoneStonecutterMenu.this.lastSoundTime != gameTime) {
                        level.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
                        EndstoneStonecutterMenu.this.lastSoundTime = gameTime;
                    }
                });
            ...
            private List<ItemStack> getRelevantItems() {
                return List.of(EndstoneStonecutterMenu.this.inputSlot.getItem());
            }
        });
    ...
    public boolean stillValid(final Player player) {
        return stillValid(this.access, player, DecoBlocks.END_STONE_STONECUTTER);
    }
