...
public class DeepslateStonecutterMenu extends AbstractContainerMenu {
    ...
    public final Container container = new SimpleContainer(1) {
        {
            Objects.requireNonNull(DeepslateStonecutterMenu.this);
        }

        @Override
        public void setChanged() {
            super.setChanged();
            DeepslateStonecutterMenu.this.slotsChanged(this);
            DeepslateStonecutterMenu.this.slotUpdateListener.run();
        }
    };
    private final ResultContainer resultContainer = new ResultContainer();

    public DeepslateStonecutterMenu(final int containerId, final Inventory inventory) {
        this(containerId, inventory, ContainerLevelAccess.NULL);
    }

    public DeepslateStonecutterMenu(final int containerId, final Inventory inventory, final ContainerLevelAccess access) {
        ...
        this.resultSlot = this.addSlot(new Slot(this.resultContainer, 1, 143, 33) {
            ...
            public void onTake(final Player player, final ItemStack carried) {
                ...
                DeepslateStonecutterMenu.this.resultContainer.awardUsedRecipes(player, this.getRelevantItems());
                ItemStack remaining = DeepslateStonecutterMenu.this.inputSlot.remove(1);
                if (!remaining.isEmpty()) {
                    DeepslateStonecutterMenu.this.setupResultSlot(DeepslateStonecutterMenu.this.selectedRecipeIndex.get());
                }

                access.execute((level, pos) -> {
                    long gameTime = level.getGameTime();
                    if (DeepslateStonecutterMenu.this.lastSoundTime != gameTime) {
                        level.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
                        DeepslateStonecutterMenu.this.lastSoundTime = gameTime;
                    }
                });
            ...
            private List<ItemStack> getRelevantItems() {
                return List.of(DeepslateStonecutterMenu.this.inputSlot.getItem());
            }
        });
    ...
    public boolean stillValid(final Player player) {
        return stillValid(this.access, player, DecoBlocks.DEEPSLATE_STONECUTTER);
    }
