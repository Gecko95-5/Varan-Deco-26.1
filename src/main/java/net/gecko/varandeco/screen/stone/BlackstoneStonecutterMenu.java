...
public class BlackstoneStonecutterMenu extends AbstractContainerMenu {
    ...
    public final Container container = new SimpleContainer(1) {
        {
            Objects.requireNonNull(BlackstoneStonecutterMenu.this);
        }

        @Override
        public void setChanged() {
            super.setChanged();
            BlackstoneStonecutterMenu.this.slotsChanged(this);
            BlackstoneStonecutterMenu.this.slotUpdateListener.run();
        }
    };
    private final ResultContainer resultContainer = new ResultContainer();

    public BlackstoneStonecutterMenu(final int containerId, final Inventory inventory) {
        this(containerId, inventory, ContainerLevelAccess.NULL);
    }

    public BlackstoneStonecutterMenu(final int containerId, final Inventory inventory, final ContainerLevelAccess access) {
        ...
        this.resultSlot = this.addSlot(new Slot(this.resultContainer, 1, 143, 33) {
            ...
            public void onTake(final Player player, final ItemStack carried) {
                ...
                BlackstoneStonecutterMenu.this.resultContainer.awardUsedRecipes(player, this.getRelevantItems());
                ItemStack remaining = BlackstoneStonecutterMenu.this.inputSlot.remove(1);
                if (!remaining.isEmpty()) {
                    BlackstoneStonecutterMenu.this.setupResultSlot(BlackstoneStonecutterMenu.this.selectedRecipeIndex.get());
                }

                access.execute((level, pos) -> {
                    long gameTime = level.getGameTime();
                    if (BlackstoneStonecutterMenu.this.lastSoundTime != gameTime) {
                        level.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
                        BlackstoneStonecutterMenu.this.lastSoundTime = gameTime;
                    }
                });
            ...
            private List<ItemStack> getRelevantItems() {
                return List.of(BlackstoneStonecutterMenu.this.inputSlot.getItem());
            }
        });
    ...
    public boolean stillValid(final Player player) {
        return stillValid(this.access, player, DecoBlocks.BLACKSTONE_STONECUTTER);
    }
    ...
    private void setupResultSlot(final int index) {
        ...
        if (!this.recipesForInput.isEmpty() && this.isValidRecipeIndex(index)) {
            SelectableRecipe.SingleInputEntry<StonecutterRecipe> entry = (SelectableRecipe.SingleInputEntry<StonecutterRecipe>)this.recipesForInput.entries().get(index);
