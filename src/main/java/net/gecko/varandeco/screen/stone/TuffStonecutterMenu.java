...
public class TuffStonecutterMenu extends AbstractContainerMenu {
    ...
    public final Container container = new SimpleContainer(1) {
        {
            Objects.requireNonNull(TuffStonecutterMenu.this);
        }

        @Override
        public void setChanged() {
            super.setChanged();
            TuffStonecutterMenu.this.slotsChanged(this);
            TuffStonecutterMenu.this.slotUpdateListener.run();
        }
    };
    private final ResultContainer resultContainer = new ResultContainer();

    public TuffStonecutterMenu(final int containerId, final Inventory inventory) {
        this(containerId, inventory, ContainerLevelAccess.NULL);
    }

    public TuffStonecutterMenu(final int containerId, final Inventory inventory, final ContainerLevelAccess access) {
        ...
        this.resultSlot = this.addSlot(new Slot(this.resultContainer, 1, 143, 33) {
            ...
            public void onTake(final Player player, final ItemStack carried) {
                ...
                TuffStonecutterMenu.this.resultContainer.awardUsedRecipes(player, this.getRelevantItems());
                ItemStack remaining = TuffStonecutterMenu.this.inputSlot.remove(1);
                if (!remaining.isEmpty()) {
                    TuffStonecutterMenu.this.setupResultSlot(TuffStonecutterMenu.this.selectedRecipeIndex.get());
                }

                access.execute((level, pos) -> {
                    long gameTime = level.getGameTime();
                    if (TuffStonecutterMenu.this.lastSoundTime != gameTime) {
                        level.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
                        TuffStonecutterMenu.this.lastSoundTime = gameTime;
                    }
                });
            ...
            private List<ItemStack> getRelevantItems() {
                return List.of(TuffStonecutterMenu.this.inputSlot.getItem());
            }
        });
    ...
    public boolean stillValid(final Player player) {
        return stillValid(this.access, player, DecoBlocks.TUFF_STONECUTTER);
    }
